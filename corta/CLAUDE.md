# CLAUDE.md — Corta
Instrucciones para el agente de código que trabaja en **esta carpeta** (`corta/`).
Las reglas comunes a todo el repo están en el [`CLAUDE.md` de la raíz](../CLAUDE.md);
acá va lo propio de Corta y manda esto dentro de `corta/`.
...
- `SPEC.md` (en esta carpeta) es el **contrato de comportamiento**. Arbitra qué es
...
- **Extra "memoria del agente"**: HECHO. La skill `/collect-memory` existe en `.claude/skills/` y se usa al cerrar sesión; la historia de git de `CLAUDE.md` muestra sus corridas. Los tres archivos de contrato (`CLAUDE.md`, `AGENTS.md`, `.github/copilot-instructions.md`) apuntan a `CLAUDE.md` como única fuente de verdad, y la ruta a la skill que citaba `copilot-instructions.md` quedó corregida (le faltaba el punto de `.claude/`).
...
- Se decidió que **si no entraron commits nuevos, la tarea no genera reporte ni commitea**: solo deja la línea en el log. Motivo: la primera versión commiteaba un reporte "sin cambios" por día y por integrante — con cuatro personas, ~120 commits vacíos por mes tapando la historia real. Beneficio extra: los días sin actividad ni siquiera invocan al modelo. `FORCE_REPORTE=1` lo genera igual, para demos.
- Se decidió mantener la cadencia **diaria** de la tarea (y no semanal). Motivo: con el punto anterior los días vacíos no dejan rastro, así que la frecuencia alta ya no tiene costo, y cuanto más seguido se dispara más fácil es mostrarla funcionando, que es lo que pide el criterio de éxito del extra.
